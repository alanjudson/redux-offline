...
class DetectNetwork {
  ...
  _setIsConnectionExpensive = async () => {
    try {
      this._isConnectionExpensive = await _netInfo.isConnectionExpensive();
    } catch (err) {
   ...
   */
  _init = async () => {
    const connectionInfo = await _netInfo.getConnectionInfo();
   ...
   */
  _addListeners() {
    _netInfo.addEventListener('connectionChange', connectionInfo => {
...
const isLegacy = typeof _netInfo.getConnectionInfo === 'undefined';
